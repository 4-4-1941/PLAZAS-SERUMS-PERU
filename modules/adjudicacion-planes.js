...
host.innerHTML=`<div class="compare-head"><div><h2>Plan A / B / C</h2><p>Organiza tus alternativas guardadas sin alterar la oferta oficial.</p></div><strong>${all.length} guardadas</strong></div><div class="sip-plan-grid">${["A","B","C"].map(p=>`<section class="sip-plan"><h3>Plan ${p}</h3>${(st.planes[p]||[]).map((r,i)=>`<article class="sip-plan-item"><b>${i+1}. ${esc(title(r))}</b><small>${esc(loc(r))}</small><button data-unplan="${esc(S().plazaKey(r))}">Quitar</button></article>`).join("")||'<div class="empty-state">Sin alternativas.</div>'}</section>`).join("")}</div><h3>Por clasificar</h3><div class="sip-unassigned">${free.map(r=>`<article class="sip-plan-item"><b>${esc(title(r))}</b><small>${esc(loc(r))}</small><div>${["A","B","C"].map(p=>`<button data-plan="${p}" data-key="${esc(S().plazaKey(r))}">Plan ${p}</button>`).join("")}</div></article>`).join("")||'<div class="empty-state">Todas tus alternativas están clasificadas.</div>'}</div>`;
host.querySelectorAll("[data-plan]").forEach(b=>b.onclick=()=>{const r=all.find(x=>S().plazaKey(x)===b.dataset.key);if(r){S().assign(b.dataset.plan,r);render()}});
